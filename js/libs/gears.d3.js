...
var Gear = {
    ...
    randomArrange: function(gears, xOffset, yOffset, angleMin, angleMax, placedGears) {
        var xx = xOffset || 0,
            yy = yOffset || 0,
            angle = 0,
            prevGear,
        ...
        placedGears.forEach(function (g) {
            let index = unplacedGears.indexOf(g);
            if (index !== -1) {
                unplacedGears.splice(index, 1);
            }
        });

        Gear.Utility.arrayShuffle(unplacedGears);
                
        if(placedGears.length <1)
        {
            // place the first unplaced gear
        ...
        for (var i = startIndex; i < unplacedGears.length; i++) {
